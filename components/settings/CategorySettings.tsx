...
// P3-67. PE TELEFON (sub 768px) fiecare categorie devine un card, iar peste 768px
    ...
    <Card className={`mb-5 ${PHONE_TABLE}`}>
      ...
      {canWrite ? (
        <form onSubmit={onAdd} className="px-5 py-4 border-b border-rc-line flex items-end gap-2">
          ...
          <Button type="submit" disabled={pending} data-testid="category-add" className={PHONE_TAP}>
      ...
      <Table>
        ...
        <tbody data-testid="category-rows">
          {categories.map((c) => (
            <tr key={c.id} data-testid="category-row" data-name={c.name} className={PHONE_ROW}>
              <Td data-label="Categorie" className={PHONE_WIDE}>
                {editingId === c.id ? (
                  <Input
                    value={editingName}
              ...
              {canWrite ? (
                <Td align="right" data-label="Acțiuni" className={PHONE_CELL}>
                  {editingId === c.id ? (
                    <span className="inline-flex gap-1.5 max-md:flex-wrap">
                      <Button
                        size="sm"
