...
### A layout change proven "desktop unchanged" by boxes, not by screenshots
**Tag:** frontend
**ERROR:** P3-67 had to prove that nine screens look the same on a computer after adding phone
classes, on a machine with no database. Pixel screenshots of the same page differ between two runs
(Next.js hydration ids, font rasterising), so a pixel diff says "changed" on untouched code, and
reading the diff for `max-md:` prefixes misses a restructured row such as the Deviz totals cells
now generated by a `.map`.
**SOLUTION:** render the pages with fixed fake data, dump every visible element inside `main` as
tag, rounded box, display, font size and `::before` content, once with the origin/main files and
